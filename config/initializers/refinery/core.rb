...
Refinery::Core.configure do |config|
  ...
  # Site name
  config.site_name = "Edge demo rev #{`git log --pretty=format:'%h' -n 1`} / Refinery CMS rev #{`cat Gemfile.lock | grep 'remote: https://github.com/refinery/refinerycms$' -A 1 | grep 'revision'`.split('revision:')[1][0..7] } / <br> Build datetime #{File.read('BUILD_DATE.txt') rescue ''}".html_safe

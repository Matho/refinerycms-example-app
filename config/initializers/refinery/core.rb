...
Refinery::Core.configure do |config|
  ...
  # Site name
  config.site_name = "Edge demo rev #{File.read('DEPLOYED_REVISION.txt')[0..6] rescue ''} / Refinery CMS rev #{`cat Gemfile.lock | grep 'remote: https://github.com/refinery/refinerycms$' -A 1 | grep 'revision'`.split('revision:')[1][0..7] }".html_safe
